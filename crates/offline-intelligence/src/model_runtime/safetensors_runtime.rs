...
impl SafetensorsRuntime {
    ...
    async fn start_server(&mut self, config: &RuntimeConfig) -> anyhow::Result<()> {
        let binary_path = config.runtime_binary.as_ref()
            .ok_or_else(|| anyhow::anyhow!("Safetensors runtime requires runtime_binary path (e.g., candle-server)"))?;

        if !binary_path.exists() {
            return Err(anyhow::anyhow!(
                "Safetensors server binary not found at: {}. Use Candle or similar framework.",
                binary_path.display()
            ));
        }

        info!("Starting Safetensors server for model: {}", config.model_path.display());
        ...
        let child = cmd.spawn()
            .map_err(|e| anyhow::anyhow!("Failed to spawn Safetensors server: {}", e))?;
        ...
        loop {
            ...
            if self.is_ready().await {
                info!("✅ Safetensors runtime ready after {:.1}s", _start.elapsed().as_secs_f64());
                return Ok(());
            }
            ...
            if elapsed_secs >= last_log_secs + 10 {
                info!("Still waiting for Safetensors server... ({}/120s)", elapsed_secs);
                last_log_secs = elapsed_secs;
            }
            delay_ms = (delay_ms * 2).min(2_000);
        }

        Err(anyhow::anyhow!("Safetensors server failed to start within 120 seconds"))
    }
}

impl Default for SafetensorsRuntime {
    fn default() -> Self {
        Self::new()
    }
}
...
#[async_trait]
impl ModelRuntime for SafetensorsRuntime {
    fn supported_format(&self) -> ModelFormat {
        ModelFormat::Safetensors
    }

    async fn initialize(&mut self, config: RuntimeConfig) -> anyhow::Result<()> {
        info!("Initializing Safetensors runtime");
        
        if config.format != ModelFormat::Safetensors {
            return Err(anyhow::anyhow!("Safetensors runtime received wrong format: {:?}", config.format));
        }
    ...
    async fn shutdown(&mut self) -> anyhow::Result<()> {
        info!("Shutting down Safetensors runtime");
        
        if let Some(mut child) = self.server_process.take() {
            match child.kill() {
                Ok(_) => {
                    info!("Safetensors server process killed successfully");
                    let _ = child.wait();
                }
                Err(e) => {
                    warn!("Failed to kill Safetensors server: {}", e);
                }
            }
        }
    ...
    fn metadata(&self) -> RuntimeMetadata {
        RuntimeMetadata {
            ...
            runtime_name: "Safetensors (Candle)".to_string(),
...
impl Drop for SafetensorsRuntime {
    fn drop(&mut self) {
        if let Some(mut child) = self.server_process.take() {
            let _ = child.kill();
            let _ = child.wait();
