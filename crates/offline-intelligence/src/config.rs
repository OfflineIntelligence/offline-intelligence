// offline-intelligence/crates/offline-intelligence/src/config.rs
...
pub struct Config {
    ...
    // ── TLS / HTTPS ─────────────────────────────────────────────────────────
    /// True when both TLS_CERT_PATH and TLS_KEY_PATH are set and non-empty.
    /// Derived automatically — do not set this directly.
    pub tls_enabled: bool,
    /// Filesystem path to the PEM-encoded TLS certificate (or full chain).
    /// Set TLS_CERT_PATH in .env to enable HTTPS.
    pub tls_cert_path: String,
    /// Filesystem path to the PEM-encoded private key matching the certificate.
    /// Set TLS_KEY_PATH in .env to enable HTTPS.
...
impl Config {
    pub fn from_env() -> Result<Self> {
        ...
        if let Ok(exe_path) = std::env::current_exe() {
            if let Some(exe_dir) = exe_path.parent() {
                ...
                // 2b. macOS: also try ~/Library/Application Support/OfflineIntelligence/.env
                //     This allows post-install configuration without modifying the bundle.
                #[cfg(target_os = "macos")]
                if !env_loaded {
                    {
                        let user_env = crate::utils::PathResolver::user_env_path();
                        if user_env.exists() {
        ...
        Ok(Self {
            ...
            prometheus_port: env::var("PROMETHEUS_PORT")
                ...
                .parse()?,
            api_host: env::var("API_HOST").unwrap_or_else(|_| {
                // Server mode: bind to all interfaces so the LAN can reach the API.
                // Desktop mode: bind to loopback only for security.
                if crate::utils::PathResolver::is_server_mode() {
                    "0.0.0.0".into()
                } else {
                    "127.0.0.1".into()
                }
            ...
            // TLS — both paths must be non-empty to enable HTTPS.
            tls_cert_path: env::var("TLS_CERT_PATH").unwrap_or_default(),
            tls_key_path: env::var("TLS_KEY_PATH").unwrap_or_default(),
            tls_enabled: !env::var("TLS_CERT_PATH").unwrap_or_default().is_empty()
                && !env::var("TLS_KEY_PATH").unwrap_or_default().is_empty(),
        })
    }
    ...
    pub fn print_config(&self) {
        ...
        if self.tls_enabled {
            info!("- TLS: ENABLED");
            info!("  Cert : {}", self.tls_cert_path);
            info!("  Key  : {}", self.tls_key_path);
        } else {
            warn!("- TLS: DISABLED — running plain HTTP. Set TLS_CERT_PATH and TLS_KEY_PATH to enable HTTPS.");
        }
    }
...
mod tests {
    ...
    fn create_tls_test_config() -> Config {
        Config {
            tls_enabled: true,
            tls_cert_path: "/etc/offline-intelligence/tls/server.crt".to_string(),
            tls_key_path: "/etc/offline-intelligence/tls/server.key".to_string(),
            ..create_test_config()
        }
    }
    ...
    fn test_apply_batch_limits_medium_context() {
        // For context 2049-4096, batch is capped at 512
    ...
    fn test_apply_batch_limits_large_context() {
        // For context 16385-32768, batch is capped at 128
    ...
    #[test]
    fn test_tls_enabled_when_both_paths_set() {
        let config = create_tls_test_config();
        assert!(config.tls_enabled);
        assert!(!config.tls_cert_path.is_empty());
        assert!(!config.tls_key_path.is_empty());
    }
    ...
    #[test]
    fn test_tls_disabled_when_only_cert_set() {
        // Both paths must be set — one alone must not enable TLS.
        let config = Config {
    ...
    #[test]
    fn test_tls_disabled_when_only_key_set() {
        let config = Config {
            tls_cert_path: String::new(),
    ...
    #[test]
    fn test_tls_cert_path_not_empty_when_enabled() {
        let config = create_tls_test_config();
        assert!(!config.tls_cert_path.is_empty(), "cert path must be set when TLS is enabled");
    }

    #[test]
    fn test_tls_key_path_not_empty_when_enabled() {
        let config = create_tls_test_config();
        assert!(!config.tls_key_path.is_empty(), "key path must be set when TLS is enabled");
    }

    #[test]
    fn test_tls_cert_and_key_are_different_paths() {
        let config = create_tls_test_config();
        assert_ne!(
            config.tls_cert_path, config.tls_key_path,
            "cert and key must be separate files"
        );
    }

    #[test]
    fn test_tls_config_clone() {
        let original = create_tls_test_config();
        let cloned = original.clone();
        assert_eq!(original.tls_enabled, cloned.tls_enabled);
        assert_eq!(original.tls_cert_path, cloned.tls_cert_path);
        assert_eq!(original.tls_key_path, cloned.tls_key_path);
    }
}
