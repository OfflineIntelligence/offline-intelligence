        ...
        #[cfg(target_os = "macos")]
        {
            ...
            loop {
                ...
                if self.is_ready().await {
                    info!("✅ CoreML runtime ready after {:.1}s", _start.elapsed().as_secs_f64());
                    return Ok(());
                }
                ...
                if elapsed_secs >= last_log_secs + 10 {
                    info!("Still waiting for CoreML server... ({}/120s)", elapsed_secs);
                    last_log_secs = elapsed_secs;
                }
                delay_ms = (delay_ms * 2).min(2_000);
            }

            Err(anyhow::anyhow!("CoreML server failed to start within 120 seconds"))
        }
    }
}
