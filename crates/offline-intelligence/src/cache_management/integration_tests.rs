...
mod tests {
    ...
    async fn test_kv_cache_lifecycle() {
        ...
        let config = KVCacheConfig {
            ...
            clear_after_conversations: 3, 
            memory_threshold_percent: 0.1, 
            bridge_enabled: true,
        ...
        let test_entries: Vec<KVEntry> = (0..50).map(|i| KVEntry {
            ...
            layer_index: (i % 8) as i32, 
            head_index: Some((i % 4) as i32), 
            importance_score: if i < 25 { 0.8 } else { 0.3 }, 
            access_count: 1,
    ...
    async fn test_memory_based_clearing() {
        ...
        let config = KVCacheConfig {
            enabled: true,
            retrieval_enabled: true,
            clear_after_conversations: 100, 
            memory_threshold_percent: 0.05, 
            bridge_enabled: true,
        ...
        let large_entries: Vec<KVEntry> = (0..20).map(|i| KVEntry {
            ...
            key_data: Some(vec![i as u8; 100]), 
            value_data: vec![i as u8; 200],    
            key_type: "attention_value".to_string(),
            layer_index: (i % 4) as i32,
            head_index: Some((i % 2) as i32),
            importance_score: if i % 3 == 0 { 0.9 } else { 0.2 }, 
            access_count: 1,
            last_accessed: chrono::Utc::now(),
    ...
    async fn test_context_continuity_via_bridge() {
        ...
        let mixed_entries: Vec<KVEntry> = (0..10).map(|i| KVEntry {
            ...
            importance_score: if i < 5 { 0.9 } else { 0.1 }, 
            access_count: 1,
            last_accessed: chrono::Utc::now(),
            token_positions: Some(vec![i as u32]),
            embedding: None,
        ...
        if let Some(clear_result) = result.clear_result {
            ...
            assert_eq!(clear_result.entries_to_keep.len(), 5); 
