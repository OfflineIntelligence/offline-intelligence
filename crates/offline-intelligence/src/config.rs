...
impl Config {
    ...
    fn auto_detect_threads() -> u32 {
        ...
        info!("Autoâ€‘detected CPU cores: {}", num_cpus);
        match num_cpus {
            1..=2 => 1,
            3..=4 => (num_cpus * 2) / 3,
            5..=8 => (num_cpus * 3) / 5,
                ...
                if device_count > 0 {
                    if let Ok(first_gpu) = nvml.device_by_index(0) {
                        if let Ok(memory) = first_gpu.memory_info() {
                            ...
                            info!("Autoâ€‘detected GPU layers: {} ({} GB VRAM)", layers, vram_gb);
                            return layers;
                        }
                    }
                }
    ...
    fn adjust_ctx_size_for_system(inferred_ctx: u32) -> u32 {
        ...
        if available_ram_gb < required_ram_gb as u64 {
            ...
            warn!(
                "Reducing context size from {} â†’ {} due to limited RAM ({}GB available)",
